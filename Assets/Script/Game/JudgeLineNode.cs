...
public partial class JudgeLineNode : Sprite2D
{
    ...
    public Dictionary<int, ChartManager.FingerData?> FingerDataDict;
    ...
    public void CalcTime(double realTime, Vector2 stageSize)
    {
        ...
        foreach (var noteObject in NoteInstances)
        {
            ...
            if (_chartTime > note.EndTime)
            {
                ...
                if (noteObject.NoteJudgeType == JudgeType.Perfect && noteObject.State == JudgeState.Judged &&
                    noteObject.NoteInfo.Type >= NoteType.Flick)
                {
                    noteObject.EmitOnJudged();
                    noteObject.State = JudgeState.Dead;
                }
                else if (noteObject.State == JudgeState.NotJudged &&
                         realTime - note.StartTime.RealTime >= ChartManager.BadRange)
                {
            ...
            var newY = StageSize.Y / 7.5f * note.Speed *
                       (float)(eventLayerList.GetCurSu(realTime) - note.FloorPosition);
            ...
            {
                if (realTime >= note.StartTime.RealTime && realTime <= note.EndTime.RealTime &&
                    noteObject.State == JudgeState.Holding)
                {
                ...
                var end = StageSize.Y / 7.5f * note.Speed *
                          (float)(eventLayerList.GetCurSu(realTime) - note.TopPosition);
                ...
                noteObject.Body.Scale = new Vector2(noteObject.Body.Scale.X,
                    (noteObject.Position.Y - end) / noteObject.Body.Texture.GetSize().Y);
            ...
            foreach (var (_, fingerData) in FingerDataDict)
            {
                ...
                if (dt < ChartManager.GoodRange &&
                    (noteObject.NoteInfo.Type == NoteType.Drag || fingerData.CurVec.Length() >= 180))
                {
                    noteObject.State = JudgeState.Judged;
                }
            }
        ...
        foreach (RpeNote noteInfo in noteList.OrEmptyIfNull())
        {
            ...
            switch (noteInfo.Type)
            {
                ...
                case NoteType.Hold:
                    instance.ZIndex = 0;
                    var (head, body, tail) =
                        instance.NoteInfo.IsHighLight ? _curPack.HoldHlTextures : _curPack.HoldTextures;
                ...
                case NoteType.Flick:
                    instance.ZIndex = 3;
                    instance.Head.Texture =
                        instance.NoteInfo.IsHighLight ? _curPack.FlickHlTexture : _curPack.FlickTexture;
                    break;
                case NoteType.Drag:
                    instance.ZIndex = 2;
                    instance.Head.Texture =
                        instance.NoteInfo.IsHighLight ? _curPack.DragHlTexture : _curPack.DragTexture;
                    break;
            }
